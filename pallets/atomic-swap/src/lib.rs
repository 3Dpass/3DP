...
// #[codec(mel_bound())]
pub struct PendingSwap<AccountId: Debug, SwapAction: Debug, BlockNumber: Debug> {
	/// Source of the swap.
...
pub mod pallet {
	...
	#[pallet::storage]
	pub type PendingSwaps<T: Config<I>, I: 'static = ()> = StorageDoubleMap<
		_,
		Twox64Concat,
		T::AccountId,
		Blake2_128Concat,
		HashedProof,
		PendingSwap<T::AccountId, T::SwapAction, T::BlockNumber>,
	>;
	...
	pub enum Event<T: Config<I>, I: 'static = ()> {
		/// Swap created.
		NewSwap { account: T::AccountId, proof: HashedProof, swap: PendingSwap<T::AccountId, T::SwapAction, T::BlockNumber> },
	...
	impl<T: Config<I>, I: 'static> Pallet<T, I> {
		...
		pub fn create_swap(
			...
			let source = ensure_signed(origin)?;
				...
				!PendingSwaps::<T, I>::contains_key(&target, hashed_proof),
			...
			PendingSwaps::<T, I>::insert(target.clone(), hashed_proof, swap.clone());
		...
		pub fn claim_swap(
			...
			ensure!(proof.len() <= T::ProofLimit::get() as usize, Error::<T, I>::ProofTooLarge);
			...
			PendingSwaps::<T, I>::remove(target.clone(), hashed_proof);
		...
		pub fn cancel_swap(
			...
			let swap = PendingSwaps::<T, I>::get(&target, hashed_proof).ok_or(Error::<T, I>::NotExist)?;
			ensure!(swap.source == source, Error::<T, I>::SourceMismatch);
			ensure!(
				frame_system::Pallet::<T>::block_number() >= swap.end_block,
				Error::<T, I>::DurationNotPassed,
			);

			swap.action.cancel(&swap.source);
			PendingSwaps::<T, I>::remove(&target, hashed_proof.clone());
