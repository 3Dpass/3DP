...
use sp_runtime::{
	traits::{
		...
		// Ensure,
		MaybeDisplay, TrailingZeroInput,
	},
	DispatchError,
	RuntimeDebug,
};
...
pub mod pallet {
	...
	use frame_support::{
		pallet_prelude::*,
		traits::{
			fungible::{Inspect as InspectFungible, Mutate as MutateFungible, Transfer},
	...
	pub trait Config: frame_system::Config {
		...
		type MultiAssetId: AssetId + Ord + From<Self::AssetId> + codec::MaxEncodedLen;
		...
		type PoolAssetId: AssetId + PartialOrd + Incrementable + From<u32> +  codec::MaxEncodedLen;
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn add_liquidity(
			...
			if reserve1 == T::AssetBalance::from(0u32) || reserve2 == T::AssetBalance::from(0u32) {
			...
			if total_supply == T::AssetBalance::from(0u32) {
		...
		pub fn remove_liquidity(
			...
			let amount2 = Self::mul_div(&lp_redeem_amount, &reserve2, &total_supply)?;
				...
				amount1 != T::AssetBalance::from(0u32) && amount1 >= amount1_min_receive,
				Error::<T>::AssetOneWithdrawalDidNotMeetMinimum
			);
			ensure!(
				amount2 != T::AssetBalance::from(0u32) && amount2 >= amount2_min_receive,
			...
			T::PoolAssets::burn_from(pool.lp_token.clone(), &sender, lp_token_burn)?; // , Precision::Exact, Fortitude::Polite)?;
	...
	impl<T: Config> Pallet<T> {
		...
		fn transfer(
			...
			let result = match T::MultiAssetIdConverter::try_convert(asset_id) {
				MultiAssetIdConversionResult::Converted(asset_id) => {
					...
					T::Assets::teleport(asset_id,from, to, amount)
				}
				MultiAssetIdConversionResult::Native => {
		...
		fn get_balance(
			...
			match T::MultiAssetIdConverter::try_convert(asset) {
				MultiAssetIdConversionResult::Converted(asset_id) => Ok(
					<<T as Config>::Assets as Inspect<T::AccountId>>::reducible_balance(asset_id, owner, true),
				),
				MultiAssetIdConversionResult::Native =>
					Self::convert_native_balance_to_asset_balance(
						<<T as Config>::Currency>::reducible_balance(owner, true),
					),
				MultiAssetIdConversionResult::Unsupported(_) =>
					Err(Error::<T>::UnsupportedAsset.into()),
			}
		...
		pub fn get_reserves(
			...
			if balance1 == T::AssetBalance::from(0u32) || balance2 == T::AssetBalance::from(0u32) {
		...
		pub fn quote_price_exact_tokens_for_tokens(
			...
			if balance1 != T::AssetBalance::from(0u32) {
				if include_fee {
		...
		pub fn get_amount_out(
			...
			if reserve_in == T::HigherPrecisionBalance::from(0) || reserve_out == T::HigherPrecisionBalance::from(0) {
		...
		fn validate_minimal_amount(
			...
			if T::MultiAssetIdConverter::is_native(asset) {
				...
				let minimal = <<T as Config>::Assets as Inspect<T::AccountId>>::minimum_balance(asset_id);
