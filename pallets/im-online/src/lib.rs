...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn tttzzz(
			...
			log::debug!(target: "runtime::im-online", "tttzzz");
		...
		pub fn heartbeat(
			...
			log::debug!(target: "runtime::im-online", "heartbeat");
	...
	impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
		fn offchain_worker(now: BlockNumberFor<T>) {
			log::debug!(target: "runtime::im-online", "offchain_worker");
	...
	impl<T: Config> ValidateUnsigned for Pallet<T> {
		...
		fn validate_unsigned(_source: TransactionSource, call: &Self::Call) -> TransactionValidity {
			log::debug!(target: "runtime::im-online", "validate_unsigned");
...
impl<T: Config> Pallet<T> {
	...
	pub fn is_online(authority_index: AuthIndex) -> bool {
		log::debug!(target: "runtime::im-online", "is_online");
	...
	fn note_authorship(author: ValidatorId<T>) {
		log::debug!(target: "runtime::im-online", "note_authorship");
	...
	pub(crate) fn send_heartbeats(
		...
		log::debug!(target: "runtime::im-online", "send_heartbeats");
			...
			T::NextSessionRotation::estimate_current_session_progress(block_number)
		{
			...
			let b = progress >= START_HEARTBEAT_FINAL_PERIOD ||
				progress >= START_HEARTBEAT_RANDOM_PERIOD && random_choice(progress);
			log::debug!(target: "runtime::im-online", "should_heartbeat_1: {}", &b);
			b
		} else {
			...
			let b = block_number >= heartbeat_after;
			log::debug!(target: "runtime::im-online", "block_number >= heartbeat_after: {}", &b);
			b
		};
	...
	fn send_single_heartbeat(
		...
		log::debug!(target: "runtime::im-online", "send_single_heartbeat");
	...
	fn local_authority_keys() -> impl Iterator<Item = (u32, T::AuthorityId)> {
		log::debug!(target: "runtime::im-online", "local_authority_keys");
		...
		log::debug!(target: "runtime::im-online", "authorities: {:?}", &authorities);
		...
		log::debug!(target: "runtime::im-online", "local_keys: {:?}", &local_keys);
	...
	fn with_heartbeat_lock<R>(
		...
		log::debug!(target: "runtime::im-online", "with_heartbeat_lock");
	...
	fn initialize_keys(keys: &[T::AuthorityId]) {
		log::debug!(target: "runtime::im-online", "initialize_keys");
...
impl<T: Config> OneSessionHandler<T::AccountId> for Pallet<T> {
	...
	{
		log::debug!(target: "runtime::im-online", "on_genesis_session");
	...
	where
		I: Iterator<Item = (&'a T::AccountId, T::AuthorityId)>,
	{
		log::debug!(target: "runtime::im-online", "on_new_session");
	...
	fn on_before_session_ending() {
		log::debug!(target: "runtime::im-online", "on_before_session_ending");
	...
	fn on_disabled(_i: u32) {
		log::debug!(target: "runtime::im-online", "on_disabled");
