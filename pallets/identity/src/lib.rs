...
pub mod pallet {
	...
	#[pallet::storage]
	#[pallet::getter(fn susp_registrars)]
	pub(super) type SuspendedRegistrars<T: Config> = StorageValue<
		_,
		BoundedVec<RegistrarIndex, T::MaxRegistrars>,
		ValueQuery,
	>;
	...
	pub enum Error<T> {
		...
		NotOwned,
		/// Registrar is suspended.
		RegistrarSuspended,
		/// Registrar is resumed.
		RegistrarResumed,
	}
	...
	pub enum Event<T: Config> {
		...
		/// A registrar was suspended.
		RegistrarSuspended { registrar_index: RegistrarIndex },
		/// A registrar was resumed.
		RegistrarResumed { registrar_index: RegistrarIndex },
	...
	impl<T: Config> Pallet<T> {
		...
		#[pallet::weight(T::WeightInfo::suspend_registrar(T::MaxRegistrars::get()))]
		pub fn suspend_registrar(
			origin: OriginFor<T>,
			#[pallet::compact] reg_index: RegistrarIndex,
		) -> DispatchResultWithPostInfo {
			T::RegistrarOrigin::ensure_origin(origin)?;

			let susp_regs = <SuspendedRegistrars<T>>::get();
			if susp_regs.contains(&reg_index) {
				return Err(Error::<T>::RegistrarSuspended.into())
			}

			let (_, registrar_count) = <SuspendedRegistrars<T>>::try_mutate(
				|susp_regs| -> Result<(RegistrarIndex, usize), DispatchError> {
					susp_regs
						.try_push(reg_index)
						.map_err(|_| Error::<T>::TooManyRegistrars)?;
					Ok(((susp_regs.len() - 1) as RegistrarIndex, susp_regs.len()))
				},
			)?;

			Self::deposit_event(Event::RegistrarSuspended { registrar_index: reg_index });

			Ok(Some(T::WeightInfo::suspend_registrar(registrar_count as u32)).into())
		}

		/// Suspend the registrar.
		#[pallet::weight(T::WeightInfo::resume_registrar(T::MaxRegistrars::get()))]
		pub fn resume_registrar(
			origin: OriginFor<T>,
			#[pallet::compact] reg_index: RegistrarIndex,
		) -> DispatchResultWithPostInfo {
			T::RegistrarOrigin::ensure_origin(origin)?;
			...
			let susp_regs = <SuspendedRegistrars<T>>::get();
			if !susp_regs.contains(&reg_index) {
				return Err(Error::<T>::RegistrarResumed.into())
			}

			let (_, registrar_count) = <SuspendedRegistrars<T>>::mutate(
				|susp_regs| -> Result<(RegistrarIndex, usize), DispatchError> {
					susp_regs.retain(|idx| *idx != reg_index);

					Ok(((susp_regs.len() - 1) as RegistrarIndex, susp_regs.len()))
				},
			)?;

			Self::deposit_event(Event::RegistrarResumed { registrar_index: reg_index });
