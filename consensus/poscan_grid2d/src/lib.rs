...
use sp_api::{Core, HeaderT, ProvideRuntimeApi, RuntimeVersion};
use sp_blockchain::HeaderBackend;
use sp_consensus_poscan::{DifficultyApi, PoscanApi};
use sp_consensus_poscan::{Seal as RawSeal, CONS_V2_SPEC_VER, SCALE_DIFF_BY, SCALE_DIFF_SINCE};
use sp_consensus_poscan::{POSCAN_ALGO_GRID2D_V3A, POSCAN_ALGO_GRID2D_V3_1, REJECT_OLD_ALGO_SINCE};
use sp_core::{crypto::Pair, hashing::blake2_256, ByteArray, H256, U256};
...
impl<B: BlockT<Hash = H256>, C, AccountId, BlockNumber> PowAlgorithm<B>
	for PoscanAlgorithm<B, C, AccountId, BlockNumber>
where
	...
	BlockNumber: Clone + Eq + Sync + Send + Debug + Encode + Decode + TypeInfo + Member + Into<u32>,
{
	...
	fn difficulty(&self, parent: B::Hash) -> Result<Self::Difficulty, Error<B>> {
		...
		self.client
			...
			.map(|d| {
				if parent_num >= SCALE_DIFF_SINCE.into() {
					d / SCALE_DIFF_BY
				} else {
					d
				}
	...
	fn verify(
		...
		match poscan_data {
			...
			_ => Err(Error::<B>::Environment(format!(
				">>> Invalid poscan data for spec_ver {}",
				ver.spec_version
			))),
		}
	}
...
impl<B: BlockT<Hash = H256>, C, AccountId, BlockNumber>
	PoscanAlgorithm<B, C, AccountId, BlockNumber>
where
	C: ProvideRuntimeApi<B> + HeaderBackend<B> + BlockBackend<B>,
	C::Api: DifficultyApi<B, U256>,
	C::Api: PoscanApi<B, AccountId, BlockNumber>,
	...
	BlockNumber: Clone + Eq + Sync + Send + Debug + Encode + Decode + TypeInfo + Member + Into<u32>,
{
	fn verify_v1(
		...
		if !hash_meets_difficulty(&seal.work, difficulty) {
			...
			info!(
				">>> work:{} poscan_hash:{} difficulty: {}",
				&seal.work, &seal.poscan_hash, difficulty
			);
			return Ok(false);
		}
		...
		let h = if poscan_data.alg_id == POSCAN_ALGO_GRID2D_V3_1
	...
	fn verify_v2(
		...
		let rotation_hash = randomx(&buf).map_err(|e| Error::Other(format!("{:#?}", e)))?;
		...
		// TODO:
		let _h = if poscan_data.alg_id == POSCAN_ALGO_GRID2D_V3_1
			|| poscan_data.alg_id == POSCAN_ALGO_GRID2D_V3A
		{
			pre_hash
		} else {
